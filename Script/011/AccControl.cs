...
public class AccControl : MonoBehaviour {
    ...
    private void OnCollisionEnter(Collision collision) {
        if (tagCheck(collision)) isJumpEnable = true;
    }

    private void OnCollisionExit(Collision collision) {
        if (tagCheck(collision)) isJumpEnable = false;
    }

    // 태그 검사 메서드                        
    private bool tagCheck(Collision collision) {
    ...
    public void setRunSpeed(float temp) { runSpeed = temp; }

    public bool getIsJumpEnable() { return isJumpEnable; }
    public void setRunLevel(int temp) { moveLevel = temp; }

    public int getMinLevel() { return _MIN_VALUE; }
}
