...
public class RaycastCam : MonoBehaviour
{
    ...
    private DrawParabola drawParabola;  // DrawParabola 스크립트 전용 
    ...
    private void Start() {
        ...
        // DrawParabola 스크립트 가져오기               
        drawParabola = GetComponent<DrawParabola>();
    }
    ...
    private void Update() {
        ...
        if(Physics.Raycast(transform.position, transform.forward, out hit, distance)) {
            ...
            if (hit.collider.isTrigger) {
                pointerObj.SetActive(false);
                drawParabola.setDrawLine(false);
                return;
            }
            ...
            // DrawParabola 컴포넌트를 이용하여 값 전달
            drawParabola.setDrawLine(true);
    ...
    // 충돌 지점을 반환하는 메소드          
    public Vector3 getCollisionPoint() { return pointerObj.transform.position; }
}
